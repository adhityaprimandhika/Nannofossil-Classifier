...
class PredictClass(Resource):
    ...
    def get(self):
        ...
        feature_value = args["feature_value"] or "30000000"
        data = []
        result = {}
        ...
        df = load_data()
        X = df[["Jumlah Lengan","Cabang Lengan","Bentuk Morfologi","Knob","Ukuran Lengan","Bentuk Lengan","Bentuk Ujung Lengan","Bentuk Ujung Lengan Melengkung"]]
        y = df["Class"]
        ...
        X_train, X_test, y_train, y_test = train_test_datasplit(X,categorized_y)
        data = np.array(data).reshape((1,8))
        prediction = predictor(data, X_train, X_test, y_train, y_test)
        transformed_prediction = label_encoder.inverse_transform(prediction.argmax(1))
